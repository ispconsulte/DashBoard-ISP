...
function DashboardKpiCardsInner({
  projects,
  activeProjects,
  totalHours,
  totalTasks,
  ...
  const kpis: KpiDef[] = [
    {
      label: "Projetos",
      value: projects.toLocaleString("pt-BR"),
      sub: activeProjects > 0 ? `${activeProjects} ativos` : "Nenhum ativo",
      icon: FolderKanban,
    ...
    },
    {
      ...
      sub: activeProjects > 0 ? `~${formatHoursHuman(totalHours / activeProjects)}/projeto` : "—",
      icon: Clock,
    ...
    },
    {
      ...
      sub: totalTasks > 0 ? `${Math.round((pendingCount / totalTasks) * 100)}% pendentes` : "—",
      icon: TrendingUp,
      accent: "hsl(45 93% 58%)",
    },
    {
      ...
      sub: overdueCount > 0 ? `${Math.round((overdueCount / totalTasks) * 100)}% — Atenção` : "Nenhum atraso ✓",
      icon: AlertTriangle,
      ...
      {kpis.map((k, i) => {
        ...
        return (
          <motion.div
            ...
            className="group relative overflow-hidden rounded-2xl border border-white/[0.06] p-4 sm:p-5 transition-all duration-500 hover:-translate-y-0.5 hover:border-white/[0.12] hover:shadow-xl w-full"
            style={{
          ...
          >
            ...
            <p className="text-[11px] font-semibold text-white/40 leading-tight text-center mb-1 uppercase tracking-wide">
              {k.label}
            </p>
            {loading ? (
            ...
            <p className="text-[10px] text-white/25 mt-0.5 text-center">
              {loading ? "Carregando…" : k.sub}
            </p>
          </motion.div>
        );
      })}
...
export default memo(DashboardKpiCardsInner);
