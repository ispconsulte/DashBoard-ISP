...
export function TaskListTable({ tasks, timeEntriesByTaskId, userNames }: TaskListTableProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-[hsl(var(--task-border))] bg-[hsl(var(--task-surface))]" style={{ minWidth: 0 }}>
      {/* Header */}
      <div className="hidden md:grid lg:hidden grid-cols-[58px_minmax(120px,1.2fr)_80px_90px_100px] bg-[hsl(var(--task-bg))] border-b border-[hsl(var(--task-border))]">
        <div className="px-2 py-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-white text-center">ID</div>
      ...
      <div className="hidden lg:grid grid-cols-[58px_minmax(140px,1.2fr)_90px_110px_170px_minmax(160px,1fr)_120px] bg-[hsl(var(--task-bg))] border-b border-[hsl(var(--task-border))]">
        <div className="px-2 py-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-white text-center">ID</div>
        <div className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-white">Tarefa</div>
        {["Status", "Prazo"].map((h) => (
          <div key={h} className="px-2 py-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-white text-center">{h}</div>
          ...
          return (
            <div key={key}>
              <motion.div
                ...
                className={`group grid grid-cols-1 md:grid-cols-[58px_minmax(120px,1.2fr)_80px_90px_100px] lg:grid-cols-[58px_minmax(140px,1.2fr)_90px_110px_170px_minmax(160px,1fr)_120px] bg-transparent transition-colors cursor-pointer hover:bg-[hsl(var(--task-surface-hover))] ${isOverdue ? "task-shake" : ""}`}
              >
                {/* Task ID */}
                <div className="hidden md:flex items-center justify-center px-1.5 py-3">
                  <span className="rounded-md border border-[hsl(var(--task-border))] bg-[hsl(var(--task-bg))] px-1.5 py-1 font-mono text-[11px] font-bold text-[hsl(var(--task-text-muted))]">
                    #{taskId ?? "—"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3 px-4 py-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-[10px] text-[hsl(var(--task-text-muted))] truncate block lg:hidden">
                      #{taskId ?? "—"} • {task.project} • {task.consultant}
                    </span>
                  </div>
