...
export function BonusMonthlyReportModal({
  ...
  // Mirrors the page gate (Sprint6BonificacaoPage): the payment manager (configured
  ...
  const hasPermission = useMemo(() => {
    // Permissao AUTOMATICA por papel: admin, gestor e responsavel geral (payment
    // manager) podem enviar para qualquer consultor, sem cadastro manual. O vinculo
    // explicito em user_coordinator_links continua valendo como fallback.
    if (session?.role === "admin" || session?.bonusRole === "admin" || session?.bonusRole === "gestor" || session?.isPaymentManager === true) return true;
    if (consultant?.userId) {
      return (session?.coordinatorOf ?? []).includes(consultant.userId);
    }
    return false;
  }, [session?.isPaymentManager, session?.role, session?.bonusRole, consultant?.userId, session?.coordinatorOf]);
      ...
      <DialogContent className="max-h-[100dvh] sm:max-h-[92vh] w-full sm:w-[95vw] max-w-3xl overflow-hidden border-white/[0.06] bg-[linear-gradient(180deg,hsl(224_35%_10%/0.98),hsl(229_33%_8%/0.98))] p-0 shadow-2xl shadow-black/50 flex flex-col sm:rounded-2xl rounded-none">
        ...
        <div className="shrink-0 border-b border-border/6">
          ...
          <motion.div
            ...
            className="flex flex-wrap items-center justify-center gap-2 border-t border-border/6 bg-white/[0.015] px-4 py-3 sm:py-3.5"
          >
            <div className="inline-flex max-w-full flex-wrap items-center justify-center gap-2.5 rounded-2xl border border-border/8 bg-white/[0.02] px-4 py-2 shadow-[0_1px_4px_rgba(0,0,0,0.15)]">
              <User className="h-3.5 w-3.5 shrink-0 text-muted-foreground/50" />
              <span className="min-w-0 max-w-[10rem] truncate text-xs font-semibold text-foreground/85">{consultant?.name ?? "Consultor"}</span>

              <div className="mx-0.5 hidden h-4 w-px bg-border/10 sm:block" />
