// ── Sprint 6.0 — Dashboard Capacidade (redesigned) ─────────────────
import { useState } from "react";
import {
  Users2,
  AlertTriangle,
  ShieldAlert,
  Clock,
  BarChart3,
  Gauge,
  UserCheck,
  PieChart,
  GraduationCap,
} from "lucide-react";
import { motion } from "framer-motion";
import { usePageSEO } from "@/hooks/usePageSEO";
import { useAuth } from "@/modules/auth/hooks/useAuth";
import { useCapacityData } from "@/modules/sprint6/hooks/useCapacityData";
import { CapacityProjectDonut } from "@/modules/sprint6/components/CapacityProjectDonut";
import { CapacityTopConsultants } from "@/modules/sprint6/components/CapacityTopConsultants";
import {
  Sprint6Filters,
  DEFAULT_SPRINT6_FILTERS,
  type Sprint6FilterState,
} from "@/modules/sprint6/components/Sprint6Filters";

import { Badge } from "@/components/ui/badge";
import PageSkeleton from "@/components/ui/PageSkeleton";
import DataErrorCard from "@/components/ui/DataErrorCard";

/* ── Animations ──────────────────────────────────────────────────── */
const fadeUp = (delay = 0) => ({
  initial: { opacity: 0, y: 14 },
  animate: { opacity: 1, y: 0 },
  transition: { duration: 0.35, delay, ease: "easeOut" as const },
});

/* ── Section Card ────────────────────────────────────────────────── */
function SectionCard({
  title,
  icon: Icon,
  children,
  badge,
}: {
  title: string;
  icon: React.ElementType;
  children: React.ReactNode;
  badge?: React.ReactNode;
}) {
  return (
    <div className="rounded-2xl border border-border/12 bg-card/40 backdrop-blur-sm overflow-hidden h-full">
      <div className="flex items-center justify-between gap-2 border-b border-border/8 px-4 py-3">
        <div className="flex items-center gap-2.5">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/10">
            <Icon className="h-3.5 w-3.5 text-primary" />
          </div>
          <h3 className="text-sm font-semibold text-foreground">{title}</h3>
        </div>
        {badge}
      </div>
      <div className="p-4 sm:p-5">{children}</div>
    </div>
  );
}

/* ── KPI Card ────────────────────────────────────────────────────── */
function KpiCard({
  label,
  value,
  sub,
  icon: Icon,
  accent,
  delay = 0,
}: {
  label: string;
  value: string;
  sub: string;
  icon: React.ElementType;
  accent: string;
  delay?: number;
}) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, delay, ease: "easeOut" as const }}
      className="group relative overflow-hidden rounded-2xl border border-white/[0.06] p-4 sm:p-5 transition-all duration-500 hover:-translate-y-0.5 hover:border-white/[0.12] hover:shadow-xl"
      style={{
        background: "linear-gradient(145deg, hsl(270 50% 14% / 0.7), hsl(234 45% 10% / 0.5))",
      }}
    >
      <div
        className="absolute top-3 right-3 h-2 w-2 rounded-full opacity-50 group-hover:opacity-100 transition-opacity"
        style={{ background: accent }}
      />
      <div className="flex items-center justify-center mb-3">
        <div
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110"
          style={{ background: `${accent.replace(")", " / 0.15)")}` }}
        >
          <Icon className="h-4 w-4" style={{ color: accent }} />
        </div>
      </div>
      <p className="text-[11px] font-semibold text-white/40 text-center mb-1 uppercase tracking-wide">
        {label}
      </p>
      <p className="text-2xl font-bold text-white/90 text-center">{value}</p>
      <p className="text-[10px] text-white/25 mt-0.5 text-center">{sub}</p>
    </motion.div>
  );
}

/* ── Compact Empty State ─────────────────────────────────────────── */
function CompactEmptyState() {
  return (
    <motion.div {...fadeUp(0.1)}>
      <div className="rounded-2xl border border-sky-500/15 bg-sky-500/[0.04] p-5">
        <div className="flex items-start gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-sky-400/10">
            <ShieldAlert className="h-4 w-4 text-sky-300" />
          </div>
          <div className="space-y-2">
            <h3 className="text-sm font-semibold text-foreground">Base operacional pendente</h3>
            <p className="text-xs text-muted-foreground leading-relaxed max-w-2xl">
              Preencha <strong className="text-foreground">elapsed_times</strong> com apontamentos vinculados e
              cadastre <strong className="text-foreground">user_capacity</strong> para o mês atual.
            </p>
            <div className="flex flex-wrap gap-2 pt-1">
              <Badge variant="outline" className="text-[9px] bg-white/[0.03] text-white/40 border-white/10">
                1. Importar apontamentos
              </Badge>
              <Badge variant="outline" className="text-[9px] bg-white/[0.03] text-white/40 border-white/10">
                2. Cadastrar capacidade
              </Badge>
              <Badge variant="outline" className="text-[9px] bg-white/[0.03] text-white/40 border-white/10">
                3. Preencher senioridade
              </Badge>
            </div>
          </div>
        </div>
      </div>
    </motion.div>
  );
}

/* ── Main Component ──────────────────────────────────────────────── */
export default function TesteCapacidadePage() {
  usePageSEO("Operacao e Capacidade");
  const { session, loadingSession } = useAuth();
  const [filters, setFilters] = useState<Sprint6FilterState>(DEFAULT_SPRINT6_FILTERS);

  const capacity = useCapacityData({
    accessToken: session?.accessToken,
    period: filters.period,
    department: filters.department,
    seniorityLevel: filters.seniorityLevel,
  });

  if (loadingSession) return <PageSkeleton variant="analiticas" />;

  if (!session?.accessToken) {
    return (
      <div className="page-gradient w-full">
        <div className="mx-auto w-full max-w-[1900px] p-4 sm:p-5 md:p-8">
          <DataErrorCard
            title="Sessão não inicializada"
            message="Faça login novamente se o problema persistir."
          />
        </div>
      </div>
    );
  }

  const utilizationAccent =
    capacity.averageUtilization != null
      ? capacity.averageUtilization > 100
        ? "hsl(0 84% 60%)"
        : capacity.averageUtilization > 80
        ? "hsl(45 93% 58%)"
        : "hsl(160 84% 39%)"
      : "hsl(0 0% 50%)";

  return (
    <div className="page-gradient w-full">
      <div className="mx-auto w-full max-w-[1900px] space-y-5 p-4 sm:p-5 md:p-8">
        {/* ── Hero Header ──────────────────────────────────────── */}
        <motion.div
          initial={{ opacity: 0, y: -18 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: "easeOut" }}
          className="relative overflow-hidden rounded-2xl border border-white/[0.07] p-5 sm:p-6 lg:p-7"
          style={{ background: "linear-gradient(145deg, hsl(222 40% 9% / 0.92), hsl(228 36% 8% / 0.72))" }}
        >
          <motion.div
            className="pointer-events-none absolute inset-y-0 right-0 w-[40%]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 1.2, delay: 0.3 }}
            style={{ background: "radial-gradient(circle at center, hsl(200 75% 50% / 0.10), transparent 65%)" }}
          />
          <div className="relative flex items-center gap-4 min-w-0">
            <motion.div
              className="relative flex h-12 w-12 items-center justify-center rounded-2xl border border-sky-400/25 bg-sky-500/10 shrink-0"
              animate={{ boxShadow: ["0 0 10px hsl(200 75% 50% / 0.08)", "0 0 24px hsl(200 75% 50% / 0.25)", "0 0 10px hsl(200 75% 50% / 0.08)"], scale: [1, 1.06, 1] }}
              transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            >
              <motion.div animate={{ rotate: [0, 15, -15, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
                <Users2 className="h-5 w-5 text-sky-400" />
              </motion.div>
            </motion.div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-foreground tracking-tight sm:text-2xl">Operação & Capacidade</h1>
              <p className="text-xs text-white/40 mt-0.5 leading-relaxed">Carga, disponibilidade e risco de sobrealocação do time</p>
            </div>
          </div>
        </motion.div>

        {/* ── Filters ─────────────────────────────────────────── */}
        <motion.div {...fadeUp(0.05)}>
          <Sprint6Filters
            filters={filters}
            onChange={setFilters}
            options={{
              departments: capacity.departments,
              showSeniority: capacity.hasSeniority,
            }}
          />
        </motion.div>

        {/* ── Alert banner ────────────────────────────────────── */}
        {!capacity.loading && capacity.overloadedCount > 0 && (
          <motion.div {...fadeUp(0.08)}>
            <div className="rounded-xl border border-rose-500/15 bg-rose-500/[0.04] px-4 py-2.5 text-xs text-rose-300/80 flex items-center gap-2">
              <ShieldAlert className="h-3.5 w-3.5 shrink-0" />
              <strong>{capacity.overloadedCount}</strong> consultor{capacity.overloadedCount > 1 ? "es" : ""} acima de 100% de utilização
            </div>
          </motion.div>
        )}

        {!capacity.loading && capacity.totalAvailableHours <= 0 && capacity.projectLoad.length > 0 && (
          <motion.div {...fadeUp(0.08)}>
            <div className="rounded-xl border border-amber-500/15 bg-amber-500/[0.04] px-4 py-2.5 text-xs text-amber-200/70 flex items-center gap-2">
              <Clock className="h-3.5 w-3.5 shrink-0" />
              Capacidade mensal não cadastrada — utilização e disponibilidade indisponíveis
            </div>
          </motion.div>
        )}

        {/* ── Error ───────────────────────────────────────────── */}
        {capacity.error && !capacity.loading && (
          <motion.div {...fadeUp(0.1)}>
            <div className="rounded-xl border border-destructive/20 bg-destructive/[0.06] px-4 py-2.5 text-xs text-destructive flex items-center gap-2">
              <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
              {capacity.error}
            </div>
          </motion.div>
        )}

        {/* ── Loading ─────────────────────────────────────────── */}
        {capacity.loading && !capacity.error && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="rounded-2xl border border-white/[0.06] p-5 animate-pulse"
                style={{ background: "linear-gradient(145deg, hsl(270 50% 14% / 0.7), hsl(234 45% 10% / 0.5))" }}
              >
                <div className="flex justify-center mb-3">
                  <div className="h-9 w-9 rounded-xl bg-white/[0.06]" />
                </div>
                <div className="h-3 w-16 mx-auto rounded bg-white/[0.06] mb-2" />
                <div className="h-6 w-12 mx-auto rounded bg-white/[0.06]" />
              </div>
            ))}
          </div>
        )}

        {/* ── Empty state ─────────────────────────────────────── */}
        {!capacity.loading && !capacity.error && !capacity.projectLoad.length && <CompactEmptyState />}

        {/* ── Dashboard Content ───────────────────────────────── */}
        {!capacity.loading && !capacity.error && capacity.projectLoad.length > 0 && (
          <>
            {/* KPI Strip */}
            <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
              <KpiCard
                label="Horas Alocadas"
                value={`${Math.round(capacity.totalHours)}h`}
                sub={`${capacity.projectLoad.length} projetos`}
                icon={Clock}
                accent="hsl(262 83% 58%)"
                delay={0.1}
              />
              <KpiCard
                label="Disponível"
                value={capacity.totalAvailableHours > 0 ? `${Math.round(capacity.totalAvailableHours)}h` : "Não encontrado"}
                sub={capacity.totalAvailableHours > 0 ? "Capacidade mensal" : "Sem cadastro"}
                icon={BarChart3}
                accent="hsl(200 80% 55%)"
                delay={0.13}
              />
              <KpiCard
                label="Utilização"
                value={capacity.averageUtilization != null ? `${capacity.averageUtilization}%` : "Não encontrado"}
                sub={
                  capacity.averageUtilization != null
                    ? capacity.averageUtilization > 100
                      ? "Sobrecarga"
                      : capacity.averageUtilization > 80
                      ? "Alta demanda"
                      : "Saudável"
                    : "Sem referência"
                }
                icon={Gauge}
                accent={utilizationAccent}
                delay={0.16}
              />
              <KpiCard
                label="Consultores"
                value={String(capacity.consultantsTracked)}
                sub="Mapeados no período"
                icon={UserCheck}
                accent="hsl(160 84% 39%)"
                delay={0.19}
              />
              <KpiCard
                label="Sobrecarregados"
                value={String(capacity.overloadedCount)}
                sub={capacity.overloadedCount > 0 ? "Acima de 100%" : "Nenhum ✓"}
                icon={ShieldAlert}
                accent={capacity.overloadedCount > 0 ? "hsl(0 84% 60%)" : "hsl(160 84% 39%)"}
                delay={0.22}
              />
            </div>

            {/* Main Grid: Donut + Consultants */}
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-5">
              <motion.div {...fadeUp(0.25)} className="lg:col-span-2">
                <SectionCard title="Carga por Projeto" icon={PieChart}>
                  <CapacityProjectDonut data={capacity.projectLoad} totalHours={capacity.totalHours} />
                </SectionCard>
              </motion.div>
              <motion.div {...fadeUp(0.28)} className="lg:col-span-3">
                <SectionCard
                  title="Consultores Mais Demandados"
                  icon={Users2}
                  badge={
                    capacity.overloadedCount > 0 ? (
                      <Badge variant="outline" className="text-[9px] bg-rose-500/10 text-rose-400 border-rose-500/20">
                        {capacity.overloadedCount} sobrecarregado{capacity.overloadedCount > 1 ? "s" : ""}
                      </Badge>
                    ) : undefined
                  }
                >
                  <CapacityTopConsultants data={capacity.topConsultants} />
                </SectionCard>
              </motion.div>
            </div>

            {/* Seniority Distribution */}
            {capacity.seniorityDistribution.length > 0 && (
              <motion.div {...fadeUp(0.3)}>
                <SectionCard title="Distribuição por Senioridade" icon={GraduationCap}>
                  <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                    {capacity.seniorityDistribution.map((item, i) => (
                      <motion.div
                        key={item.level}
                        initial={{ opacity: 0, y: 8 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.32 + i * 0.05, duration: 0.25 }}
                        className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 hover:bg-white/[0.04] transition-colors"
                      >
                        <p className="text-[11px] font-semibold text-white/40 uppercase tracking-wide">{item.level}</p>
                        <p className="mt-1.5 text-xl font-bold text-white/90">{item.hours}h</p>
                        <p className="text-[10px] text-white/25 mt-0.5">
                          {item.consultants} consultor{item.consultants > 1 ? "es" : ""}
                        </p>
                      </motion.div>
                    ))}
                  </div>
                </SectionCard>
              </motion.div>
            )}
          </>
        )}
      </div>
    </div>
  );
}
