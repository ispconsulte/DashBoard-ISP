...
export default function TarefasPage() {
  ...
  const hasActiveFilters =
    ...
    deadline !== "all" ||
    period !== "all" ||
    dateFilterMode !== PAGE_DEFAULT_DATE_FILTER_MODE ||
    consultant !== "all" ||
    project.length > 0 ||
    !!dateFrom ||
    !!dateTo ||
  ...
  const resetFilters = useCallback(() => {
    ...
    setPeriod("all"); setDateFilterMode(PAGE_DEFAULT_DATE_FILTER_MODE); setConsultant("all"); setProject([]);
  ...
  return (
    <div className="page-gradient w-full">
      <div className="mx-auto w-full max-w-[1900px] space-y-5 p-4 sm:p-5 md:p-8 overflow-x-hidden">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2.5 text-[11px] flex-wrap mt-0.5">
                            ...
                            {data.hours > 0 && (
                              <span className="inline-flex items-center gap-1 font-medium text-[hsl(220_90%_66%)]">
                                <Clock className="h-3 w-3" />
                                {formatSecondsHuman(Math.round(data.hours * 3600))}
                              </span>
                            )}
                          </div>
                        </div>
        ...
        {/* ═══ TASK LIST ═══ */}
        <ActivityListSection total={filteredTasks.length}>
          {(error || timesError) && (
